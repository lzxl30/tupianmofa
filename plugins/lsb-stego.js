  ...
  render(container) {
    container.innerHTML = `
      ...
      <div class="result-area" id="stegoEmbedResult">
        ...
        <img id="stegoEmbedPreview">
        <div class="btn-row"><a class="btn btn-accent" id="stegoDownloadBtn" download="hidden_image.png">⬇️ 下载含密图片</a></div>
      </div>
      <div class="result-area" id="stegoExtractResult">
        <p style="color:var(--accent2);">🔓 提取成功！隐藏的图片：</p>
        <img id="stegoExtractPreview">
        <div class="btn-row"><a class="btn btn-accent" id="stegoExtractDownloadBtn" download="extracted_secret.png">⬇️ 下载提取的图片</a></div>
      </div>
      <div class="tip-bar">
...
function initStegoEvents(container) {
  ...
  setupUpload('stegoCarrierZone','stegoCarrierInput','stegoCarrierClear', (i)=>{ carrierImg=i; updateBtn(); });
  setupUpload('stegoSecretZone','stegoSecretInput','stegoSecretClear', (i)=>{ secretImg=i; updateBtn(); });
  ...
  function embedLSB(cover, secret) {
    ...
    const coverData=ctx.getImageData(0,0,cw,ch); const coverPixels=coverData.data;
    ...
    if(needed>cw*ch*3) throw new Error('载体太小，无法隐藏秘密图片');
  ...
  function extractLSB(imageData, w, h) {
    ...
    const bits=[];
    for(let i=0;i<pixels.length;i+=4){ bits.push(pixels[i]&1, pixels[i+1]&1, pixels[i+2]&1); }
  ...
  embedBtn.addEventListener('click', ()=>{
    ...
    try {
      ...
      container.querySelector('#stegoEmbedResult').classList.add('show');
      container.querySelector('#stegoExtractResult').classList.remove('show');
      container.querySelector('#stegoEmbedPreview').src = url;
      container.querySelector('#stegoDownloadBtn').href = url;
      container.querySelector('#stegoEmbedResult').scrollIntoView({ behavior:'smooth', block:'center' });
      showToast('🔐 隐藏成功！');
  ...
  container.querySelector('#stegoExtractBtn').addEventListener('click', ()=>{
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        container.querySelector('#stegoExtractResult').classList.add('show');
        container.querySelector('#stegoEmbedResult').classList.remove('show');
        container.querySelector('#stegoExtractPreview').src = secretUrl;
        container.querySelector('#stegoExtractDownloadBtn').href = secretUrl;
        container.querySelector('#stegoExtractResult').scrollIntoView({ behavior:'smooth', block:'center' });
        showToast('🔓 提取成功！');
      } catch(e) { showToast('❌ '+e.message); }
    };
    input.click();
